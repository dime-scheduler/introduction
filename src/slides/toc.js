...
export default function TableOfContents() {
    const tableOfContentsList = tableOfContentsData.map(({ pageNo, chapter }, i) => {
        const slideUrl = `#slide=${pageNo}`;
        return (
            <li key={i}>
                <a href={slideUrl} title={chapter}>
                    <span className="chapter">{chapter}</span>
                    <span className="toc-page">{pageNo}</span>
                </a>
            </li>
        )
    });
    ...
    return (
        <section>
            <div className="wrap size-50">
                <h3><FormattedMessage id="toc.title" defaultMessage="Table of contents" /></h3>
                <hr />
                <div className="toc">
